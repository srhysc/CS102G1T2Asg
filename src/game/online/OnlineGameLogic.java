...
/**
 * The OnlineGameLogic class contains the core game logic for managing an online
 * multiplayer game.
 * It handles player turns, game state updates, and communication between the
 * server and clients.
 *
 * Key Responsibilities:
 * - Manage the flow of the game, including player turns and game-ending
 * conditions.
 ...
 *
 * Key Features:
 * - Supports interactive gameplay for multiple players in an online
 * environment.
 * - Handles the final round and endgame logic, including score calculation.
 * - Provides utility methods for broadcasting messages and managing player
 * input.
 ...
 * - {@link SocketPacket} – Used for communication between the server and
 * clients.
 *
 * Assumptions:
 * - The server is running and all players are connected before the game starts.
 * - The game ends when all non-host players disconnect or the game logic
 * concludes.
 *
 * Utility Methods:
 * - {@code playOnlineTurn} – Manages the main game loop for online gameplay.
 * - {@code onlineLastRound} – Handles the logic for the final round of the
 * game.
 * - {@code endGameOnline} – Processes the endgame logic, including discarding
 * cards and scoring.
 * - {@code broadcastToAll} – Sends messages or updates to all connected
 * players.
 * - {@code flushInputBuffer} – Clears any unwanted input from the buffer.
 * - {@code handlePlayerDisconnection} – Handles the removal of disconnected
 * players and adjusts the game state.
 * - {@code buildMenuMessage} – Constructs the menu message displayed to players
 * during their turn.
 */
...
public class OnlineGameLogic {
            ...
            try { // to manage player disconnects
                ...
                while (!deck.isEmpty()) {
                    ...
                    if (currentPlayer.getInputSteam() == null) {
                        ...
                        SocketPacket moveResponse = (SocketPacket) currentPlayer.getInputSteam().readObject(); // wait
                                                                                                               // and
                                                                                                               // read
                                                                                                               // online
                                                                                                               // game
     ...
     * in the game.
     * - {@code playerList} – A list of {@link Player} objects representing the
     * players in the game.
     * - {@code turnManager} – The {@link TurnManager} object managing the turn
     * order.
     * - {@code isTwoPlayerGame} – A boolean indicating whether the game is a
     * two-player game.
     * - {@code outputs} – A list of {@link ObjectOutputStream} objects for sending
     * data to clients.
     * - {@code inputs} – A list of {@link ObjectInputStream} objects for receiving
     * data from clients.
        ...
        while (!GameServer.playersSockets.isEmpty()) {
            try {
                ...
                for (int i = 0; i < playerList.size(); i++) {
                    ...
                    String takenCardsString = takenCards.stream().map(card -> card.toString())
    ...
    /**
     * Processes the endgame logic, including discarding cards and calculating
     * scores.
     ...
     * - {@code playerList} – A list of {@link Player} objects representing the
     * players in the game.
     * - {@code sp} – A {@link SocketPacket} object containing the message or update
     * to broadcast.
     *
     * Exceptions:
    ...
    /**
     * Clears any unwanted input from the input buffer to prevent accidental input
     * values.
    ...
    /**
     * Handles the removal of disconnected players and adjusts the game state
     * accordingly.
     * Ensures that the game continues with the remaining players or ends if all
     * players disconnect.
     *
     * Parameters:
     * - {@code currentPlayer} – The {@link Player} object representing the
     * disconnected player.
     * - {@code playerList} – A list of {@link Player} objects representing the
     * players in the game.
     * - {@code outputs} – A list of {@link ObjectOutputStream} objects for sending
     ...
     */
    private static boolean handlePlayerDisconnection(Player currentPlayer, ArrayList<Player> playerList,
            List<ObjectOutputStream> outputs, List<ObjectInputStream> inputs) {
     ...
     * Includes information about the deck, parade, current player, collected cards,
     * and hand.
     *
     * Parameters:
     * - {@code deck} – The {@link Deck} object representing the deck of cards used
     * in the game.
     * - {@code currentPlayer} – The {@link Player} object representing the current
     * player.
     *
     * Returns:
