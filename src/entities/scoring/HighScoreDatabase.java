 ...
 * 
 * This class is responsible for loading, saving, and updating the high scores
 * of players.
 ...
 * - {@code loadScores()} loads the existing scores from the file when the game
 * starts.
 * - {@code saveScores()} saves the updated scores back to the file.
 * - {@code updateHighScore(String playerName)} increases a player's win count
 * and saves it.
 * - {@code displayHighScores()} shows the current high scores, highlighting the
 * top 3.
 * 
 * File format:
 * - Each line in the "WinsDatabase.txt" file represents one player's score in
 * this format:
 * "Player: [playerName], Wins: [winsCount]".
 * 
 * Notes:
 * - CPU players are ignored in the high score list.
 * - The scores are sorted in descending order, with the top player shown first
 * in gold.
 */
